...
float modeling_random_x(float nu_coef, float mu_coef, float lambda_coef) {
    ...
    return lambda_coef * sqrt(1 - pow(realisation_of_value_1, 1 / (nu_coef + 0.5))) * cos(
               2 * M_PI * realisation_of_value_2) + mu_coef;
}

float get_random_from_0_to_1() {
    float realisation_of_value;
    do realisation_of_value = (float) rand() / RAND_MAX; while (
        realisation_of_value == 0. || realisation_of_value == 1.);
...
float get_density_by_coefs(float x, float nu_coef, float mu_coef, float lambda_coef) {
    return 1.0 / (lambda_coef * 2 * beta(nu_coef + 1, nu_coef + 1)) * pow(
               (1 - pow(((x - mu_coef) / lambda_coef), 2)) / 4, nu_coef);
}
...
float get_mix_excess_by_coefs(float nu_coef_1, float mu_coef_1, float lambda_coef_1, float nu_coef_2,
    ...
    float element_1 = (1 - p) * (pow(mu_coef_1 - expectation, 4) + 6 * pow(mu_coef_1 - expectation, 2) * dispersion_1 +
                                 4 * (
                                     mu_coef_1 - expectation) * pow(dispersion_1, 1.5) * asymmetry_1 + pow(
                                     dispersion_1, 2) * (excess_1 + 3));
    float element_2 = p * (pow(mu_coef_2 - expectation, 4) + 6 * pow(mu_coef_2 - expectation, 2) * dispersion_2 + 4 * (
                               mu_coef_2 - expectation) * pow(dispersion_2, 1.5) * asymmetry_2 + pow(dispersion_2, 2) *
                           (excess_2 + 3));
...
int get_density_array(float *dataset_begin, float *dataset_end, float *&number_of_occurrences_array_begin,
...
float *modeling_sample_based_on_sample(int sample_volume, float *dataset_begin, float *dataset_end) {
    ...
    int number_of_intervals = get_density_array(dataset_begin, dataset_end, number_of_occurrences_array_begin,
                                                number_of_occurrences_array_end);
