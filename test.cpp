...
void test() {
    ...
    // }
    // cout << "-------------------------------------------------------------------------------" << endl;
    // cout << "ТЕСТИРОВАНИЕ КЛАССА ЕМПИРИЕСКОГО РАСПРЕДЕЛНИЯ ИЗ ФАЙЛА" << endl;
    // cout << "-------------------------------------------------------------------------------" << endl;
    // EmpiricDistribution *ED;
    // int dataset_len;
    // float *dataset;
    // ifstream file("distribution_test.data");
    //
    // if (not file.is_open()) {
    ...
    // file.close();
    // try {
    //     ED = new EmpiricDistribution("distribution_test.data");
    ...
    //     double expectation_class = ED->get_expectation();
    //     double density_class = ED->get_density(expectation_class);
    //     double assym_class = ED->get_asymmetry();
    //     double excess_class = ED->get_excess();
    //     double dispersion_class = ED->get_dispersion();
    //
    ...
    //             dataset[j] = GD.modeling_random_x();
    //         }
    //         ED = new EmpiricDistribution(GD, array_dataset_len[i]);
    //
    ...
    //         double expectation_class = ED->get_expectation();
    //         double density_class = ED->get_density(array_mu[i]);
    //         double assym_class = ED->get_asymmetry();
    //         double excess_class = ED->get_excess();
    //         double dispersion_class = ED->get_dispersion();
    //
    ...
    //         dataset = new float[array_dataset_len[i]];
    //         for (int j = 0; j < array_dataset_len[i]; j++) {
    //             dataset[j] = MD.modeling_random_x();
    //         }
    //         float mu = MD.get_expectation();
    //
    ...
    //         double dispersion_dataset = get_dispersion_by_dataset(dataset, dataset + array_dataset_len[i]);
    //         double expectation_class = ED->get_expectation();
    //         double density_class = ED->get_density(mu);
    //         double assym_class = ED->get_asymmetry();
    //         double excess_class = ED->get_excess();
    //         double dispersion_class = ED->get_dispersion();
    //
    //         assert(abs(expectation_dataset - expectation_class) <= 1.0);
    //         assert(abs(density_dataset - density_class) <= 1.0);
    //         assert(abs(assym_dataset - assym_class) <= 1.0);
    //         assert(abs(excess_dataset - excess_class) <= 1.0);
    //         assert(abs(dispersion_dataset - dispersion_class) <= 1.0);
    //
    //         cout << "Тест №" << i+1 << endl;
    //         cout << "Мат ожидание: " << expectation_dataset << '\t' << expectation_class << endl;
    //         cout << "Плотность в мат ожидании: " << density_dataset << "\t" << density_class << endl;
